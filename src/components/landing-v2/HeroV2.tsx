    ...
    <section id="hero" className="relative w-full min-h-[70vh] md:min-h-[80vh] flex items-center justify-center px-4 md:px-6 py-16 md:py-24 overflow-hidden">
      {data.imageDesktop && (
        <>
          <img
            ...
            alt=""
            fetchPriority="high"
            className={`absolute inset-0 w-full h-full object-cover ${posClass} hidden md:block`}
          />
          {data.imageMobile && (
            <img
              ...
              alt=""
              fetchPriority="high"
              className={`absolute inset-0 w-full h-full object-cover ${posClass} md:hidden`}
            />
          )}
      ...
      <div className="max-w-5xl mx-auto text-center relative z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6 mt-6 md:mt-10">
          {data.ctaPrimary && (<a href={applyUTMv2(data.ctaPrimary.link, lpKey)} className="w-full sm:w-auto px-10 md:px-14 py-4 md:py-5 rounded-full font-bold text-base md:text-lg transition-transform duration-300 bg-[hsl(var(--ds-color-btn))] text-[hsl(var(--ds-color-btn-text))] hover:scale-105 hover:shadow-[0_16px_32px_hsl(var(--ds-color-accent)/0.4)] shadow-[0_8px_16px_hsl(var(--ds-color-accent)/0.2)]">{data.ctaPrimary.text}</a>)}
          {data.ctaSecondary && (<a href={applyUTMv2(data.ctaSecondary.link, lpKey)} className="w-full sm:w-auto px-10 md:px-14 py-4 md:py-5 rounded-full font-bold text-base md:text-lg transition-transform duration-300 border-2 border-[hsl(var(--ds-color-accent))] text-[hsl(var(--ds-color-accent))] bg-transparent hover:bg-[hsl(var(--ds-color-accent)/0.1)] hover:scale-105">{data.ctaSecondary.text}</a>)}
        </div>
      </div>
